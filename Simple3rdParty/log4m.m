classdef log4m < handle 
    ...
    methods (Static, Access=private)
        function obj = activeLogger(logPath, reset)
            persistent localObj;
            if isempty(localObj) || ~isvalid(localObj) || (nargin >= 2 && reset)
    ...
    methods (Static)
        function obj = forceNewLogger(logPath)
            if(nargin == 0)
                obj = getLogger();
            else
                obj = log4m.activeLogger(logPath, true);
            end
        end
        ...
        function obj = getLogger( logPath )
            ...
            obj = log4m.activeLogger(logPath);
        end
...
%% The public Logging methods %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function log(self, funcName, message, loglevel)
            ...
            % 
            self.writeLog(loglevel,funcName,message);
        end
